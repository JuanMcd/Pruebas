...
int LimiteTemperatura = Default_Temp;
...
//-------------------------------------------------------
// Definir los GPIO conectados con los LED, al sensor y un bool para la conexión a WiFI
static uint8_t LED1 = 23;  //D23
static uint8_t LED2 = 22;  //D22
static uint8_t PIN_DHT= 4; 
//ventilador
static uint8_t PIN_Ventilador = 18; //toca poner el que esté en el montaje
...
bool ESTADO_Ventilador = LOW;// define el estado del ventilador
//---------------------------------------------------
...
static Switch Ventilador(dispositivo3, &PIN_Ventilador);
static Fan Selector("Selector");
#define NIVEL_SELECTOR_POR_DEFECTO 0
//---------------------------------------------------
...
void write_callback(Device *device, Param *param, const param_val_t val, void *priv_data, write_ctx_t *ctx)
{
    ...
    else if (strcmp(nombre_dispositivo, dispositivo3) == 0)
    {
     ...
     //si "Temperatura"
    else if (strcmp(nombre_dispositivo, "Selector") == 0) //aca no se si es este o temperatureSENSOR
    {
      if (strcmp(nombre_parametro, "Power") == 0){
        int fan_on_off_value = val.val.b;
        Serial.printf("\nValor Recibido = %d for %s - %s\n",fan_on_off_value, nombre_dispositivo, nombre_parametro);
      }else if (strcmp(nombre_parametro, "Level")==0){
        int LimiteTemperatura = val.val.i;
        Serial.printf("\nValor recibido = %d para %s - %s\n", LimiteTemperatura, nombre_dispositivo, nombre_parametro);
        //delay(15);
      }
...
void setup(){
  ...
  /*Param level_param("Level", "custom.param.level", value(NIVEL_SELECTOR_POR_DEFECTO), PROP_FLAG_READ | PROP_FLAG_WRITE);
  level_param.addBounds(value(18), value(27), value(1)); //sart_value, end_value, interval
